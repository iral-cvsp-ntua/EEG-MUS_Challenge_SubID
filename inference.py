...
def parse():
    ...
    # Dataset options
    parser.add_argument('--run_name',type=str,default='myrun')
    ...
    parser.add_argument('--splitnum',type=str, default='0') 
    ...
    parser.add_argument('--trainer', default='infer')
...
def main(args):
    ...
    def train_func():
        ...
        if args.sweep:
            ...
            WANDB_MODE = 'dryrun'
            # Init run
            run = wandb.init(project=args.task, entity=args.entity,mode='offline')
            run.config.value = 3
        ...
        trainer_class = getattr(trainer_module, 'Inference')
        ...
        #best_idx = np.argmax(np.asarray(metrics['valid']['balanced_accuracy']))
        #print('best epoch',best_idx)
        #print('testing accuracy at best validation epoch',metrics['test']['balanced_accuracy'][best_idx])        
        #print('validation accuracy at best validation epoch',metrics['valid']['balanced_accuracy'][best_idx])
