...
class Spectrum(object):
    '''
    Calculates the FFT magnitude of a sample
    '''
    def __call__(self,sample):
        eeg, label = sample['eeg'],sample['label']
        eeg = np.log(np.abs(np.fft.fft(eeg,N=1280))+10e-6)
        return{'eeg':eeg, 'label':label}
...
class AddNoise(object):
    ...
    '''
    adds gaussian noise to a normalized object
    '''
    def __call__(self,sample):
        eeg, label= sample['eeg'], sample['label']
        #print(eeg.shape,eeg.mean(),eeg.std())
        eeg += 0*torch.randn(eeg.shape[0],eeg.shape[1])#,eeg.shape[2])
